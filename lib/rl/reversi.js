	...
	get actions() {
		...
		for (let i = 0; i < this._size[0]; i++) {
			for (let j = 0; j < this._size[1]; j++) {
				a.push(`${String.fromCharCode('a'.charCodeAt(0) + i)}${j + 1}`)
			}
		}
		return [a]
	}
	...
	test(state, action, agent) {
		...
		if (action[0] === EMPTY) {
			...
			return new RLStepResult(
				this,
				invalid ? state : this._makeState(board, agent, flipPiece(gameturn)),
				getreward(done),
				done,
				invalid
			)
		...
		return new RLStepResult(this, this._makeState(board, agent, flipPiece(gameturn)), getreward(done), done)
	}
}
