...
export default class PROCLUS {
	...
	fit() {
		...
		const D = this._findDimensions(L)
		const C = this._assignPoints(D)
	...
	_findDimensions(L) {
		...
		for (let i = 0; i < this._k; i++) {
			...
			for (let k = 0; k < L[i].length; k++) {
				for (let d = 0; d < dim; d++) {
					x[d] += Math.abs(this._x[this._mcurrent[i]][d] - this._x[L[i][k]][d])
				}
			}
			for (let d = 0; d < dim; d++) {
				x[d] /= L[i].length
	...
	_assignPoints(D) {
		...
		for (let i = 0; i < this._x.length; i++) {
			...
			let min_k = -1
			for (let k = 0; k < this._k; k++) {
				const d = D[k].reduce((s, d) => s + Math.abs(this._x[i][d] - this._x[this._mcurrent[k]][d]), 0)
				if (d < min_d) {
					min_d = d
					min_k = k
				}
	 ...
	 */
	predict() {
		this._D = this._findDimensions(this._clusters)
		const C = this._assignPoints(this._D)
		const p = []
