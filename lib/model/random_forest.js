...
export default class RandomForest {
	...
	/**
	 * @param {number} tree_num
	 * @param {number} sampling_rate
	 * @param {DecisionTree} tree_class
	 * @param {*[]} tree_class_args
	 */
	constructor(tree_num, sampling_rate = 0.8, tree_class, tree_class_args = null) {
		...
		this._tree_class_args = tree_class_args || []
	}
	...
	init(datas, targets) {
		...
		for (let i = 0; i < this._treenum; i++) {
			...
			const tree = new this._treeclass(...this._tree_class_args)
