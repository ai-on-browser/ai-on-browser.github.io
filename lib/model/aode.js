...
class ODE {
	...
	fit(datas, y, k) {
		...
		for (let i = 0; i < datas.length; i++) {
			xkmax = Math.max(xkmax, datas[i][k])
			xkmin = Math.min(xkmin, datas[i][k])
		}
		this._r = [-Infinity]
		...
		for (let n = 0; n < this._labels.length; n++) {
			...
			for (let t = 0; t < this._discrete; t++) {
				...
				if (data.length >= this._m) {
					...
					this._p[n][t]._estimate_prob(data, n, t)
			...
			const p = data.map(d => {
				for (let t = 0; t < this._discrete; t++) {
					if (this._r[t] <= d[this._k] && d[this._k] < this._r[t + 1]) {
						...
						return this._p[i][t]._data_prob(d, i, t) * this._rate[i][t]
					}
				}
				return 0
			})
