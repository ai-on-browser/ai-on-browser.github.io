	...
	fit(x, y) {
		...
		if (this._method === 'ISTA') {
			...
			while (ias.length > 0) {
				...
				if (!change_sign_flag) {
					j = ias[c.row(ias).copyMap(Math.abs).argmax(0).toScaler()]
					as.push(j)
					ias.splice(ias.indexOf(j), 1)
				}
				const C = c.copyMap(Math.abs).argmax(0).toScaler()
				...
				const lmb = (Math.abs(x.col(idx).tDot(y.copySub(x.dot(tbeta))).toScaler()) * 2) / x.rows

				if (lmb < this._lambda) {
					const pl = (Math.abs(x.col(idx).tDot(y.copySub(x.dot(this._w))).toScaler()) * 2) / x.rows
