...
export default class SplitAndMerge {
	...
	_shouldSplit(data) {
		...
		if (this._method === 'variance') {
			...
			let m = 0
			for (let i = 0; i < n; i++) {
				mean[i] = data[i].reduce((s, v) => s + v, 0) / d
				m += mean[i]
			}
			m /= n
			let max_v = -Infinity
			for (let i = 0; i < n; i++) {
				const v = Math.abs(mean[i] - m)
				max_v = Math.max(max_v, v)
			}
			if (max_v > this._threshold) {
				return true
			}
		}
		return false
			...
			for (let j = 0; j < segments.length; j++) {
				...
				for (let k = 0; k < r.length; k++) {
					if (r[k][0] === range[k][1] || r[k][1] === range[k][0]) {
						if (r.every((_, d) => d === k || (r[d][0] < range[d][1] && range[d][0] < r[d][1]))) {
							adjust = true
						}
					}
				}
