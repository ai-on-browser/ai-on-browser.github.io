...
export default class EnsembleBinaryModel {
	...
	constructor(model, type, classes) {
		if (type === 'oneone') {
			this._model = new OneVsOneModel(model, classes)
		} else if (type === 'onerest') {
			this._model = new OneVsRestModel(model, classes)
		}
	}
	...
	init(train_x, train_y) {
		this._model.init(train_x, train_y)
	}
...
class OneVsRestModel {
	...
	predict(data) {
		const pred = []
		for (let i = 0; i < this._n; i++) {
			this._model[i].predict(data).forEach((v, k) => {
				if (!pred[k]) {
					pred[k] = []
				}
...
class OneVsOneModel {
	...
	predict(data) {
		...
		for (let i = 0; i < this._n; i++) {
			for (let j = 0; j < i; j++) {
				this._model[i][j].predict(data).forEach((v, k) => {
					pred[k][v > 0 ? i : j]++
				})
			}
		}
