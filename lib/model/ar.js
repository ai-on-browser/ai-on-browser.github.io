...
export default class AR {
	...
	_yuleWalker(x) {
		...
		this._phi = G.solve(g)
		this._variance = G.at(0, 0) - this._phi.tDot(g).value[0]
	}
	...
	_householder(x) {
		...
		const sx = s.slice(0, 0, this._p, this._p)
		const sy = s.slice(0, this._p, this._p, this._p + 1)
	...
	predict(data, k) {
		...
		for (let i = 0; i < k; i++) {
			...
			const pred = last.dot(this._phi).value[0]
			preds.push(pred)
			lasts.unshift(pred)
			lasts.pop()
		}
