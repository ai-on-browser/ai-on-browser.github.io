...
export default class COLL {
	...
	fit() {
		...
		for (let l = 0; l < n; l++) {
			...
			pi[min_nu].push(i)
			for (let j = 0; j < n; j++) {
				this._w.set(min_nu, j, (1 - this._tau) * this._w.at(min_nu, j) + this._tau * this._k[i][j])
			}
			this._w.set(
				min_nu,
				n,
				(1 - this._tau) ** 2 * this._w.at(min_nu, n) +
					this._tau ** 2 * this._k[i][i] +
					2 * (1 - this._tau) * this._tau * this._w.at(min_nu, i)
			)
			this._nk[min_nu] += 1
