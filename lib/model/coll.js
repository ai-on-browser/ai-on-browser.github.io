...
export default class COLL {
	...
	fit() {
		...
		for (let l = 0; l < n; l++) {
			...
			pi[min_nu].push(i)
			for (let j = 0; j < n; j++) {
				this._w.set(min_nu, j, (1 - this._eta) * this._w.at(min_nu, j) + this._eta * this._k[i][j])
			}
			this._w.set(
				min_nu,
				n,
				(1 - this._eta) ** 2 * this._w.at(min_nu, n) +
					this._eta ** 2 * this._k[i][i] +
					2 * (1 - this._eta) * this._eta * this._w.at(min_nu, i)
			)
			this._nk[min_nu] += 1
		...
		let err = 0
		for (let k = 0; k < this._c; k++) {
			err += (1 - 1 / (1 - this._eta) ** pi[k].length) ** 2 * this._w.at(k, n)
			for (let h = 0; h < pi[k].length; h++) {
				for (let l = 0; l < pi[k].length; l++) {
					err += (this._eta ** 2 * this._k[pi[k][h]][pi[k][l]]) / (1 - this._eta) ** (h + l)
				}
			}
			const s = 2 * this._eta * (1 - 1 / (1 - this._eta) ** pi[k].length)
			for (let l = 0; l < pi[k].length; l++) {
				err += (s * this._w.at(k, pi[k][l])) / (1 - this._eta) ** l
			}
		}
		return err
	}
