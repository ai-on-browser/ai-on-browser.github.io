...
export default {
	...
	export(model, obj, info) {
		...
		if (typeof obj.scale === 'string') {
			...
			const scale = Array.isArray(obj.scale) ? obj.scale : Array(size[channelDim] ?? 1).fill(obj.scale ?? 1)
		...
		if (typeof obj.offset === 'string') {
			...
			const offset = Array.isArray(obj.offset) ? obj.offset : Array(size[channelDim] ?? 1).fill(obj.offset ?? 0)
