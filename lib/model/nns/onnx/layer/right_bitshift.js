...
/**
 * Handle right bitshift layer
 */
export default {
	/**
	 * Export to onnx object.
	 * @param {onnx.ModelProto} model Model object
	 * @param {import("../../graph").LayerObject & {type: 'right_bitshift'}} obj Node object
	...
	export(model, obj, info) {
		...
		attrDirection.setS(new TextEncoder().encode('RIGHT'))
		...
		for (let i = 1; i < intInputs.length - 1; i++) {
			...
			prev_in = `${obj.name}_bitshift_${i - 1}`
			node_shift.addOutput(prev_in)
