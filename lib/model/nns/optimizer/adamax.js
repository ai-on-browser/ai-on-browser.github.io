...
export class AdaMaxOptimizer {
	constructor(lr = 0.002, beta1 = 0.9, beta2 = 0.999) {
	...
	manager() {
		...
		return {
			...
			delta(key, value) {
				...
				this.params[key].s.broadcastOperate(value, (a, b) => Math.max(a * this_._beta2, Math.abs(b)))
				const nv = 1 - this_._beta1 ** this.params[key].c
				const ret = this.params[key].v.copy()
				ret.broadcastOperate(this.params[key].s, (a, b) => (a / nv) * (this.lr / b))
				this.params[key].c++
