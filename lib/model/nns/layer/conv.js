						...
						for (let n = 0; n < outSize[2]; n++) {
							...
							for (let c = 0; c < this._in_channel; c++) {
								for (let s = 0; s < this._kernel[0]; s++) {
									const si = m * this._stride - this._padding + s
									if (si < 0 || si >= x.sizes[1]) {
										continue
									}
									for (let t = 0; t < this._kernel[1]; t++) {
										const ti = n * this._stride - this._padding + t
										if (ti < 0 || ti >= x.sizes[2]) {
											continue
										}
										v += x.at(i, si, ti, c) * this._w.at(c, s, t, b)
									}
								}
							}
