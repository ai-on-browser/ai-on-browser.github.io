...
const simpleLayers = {
	abs: {
		calc: Math.abs,
		grad: (i, o) => Matrix.map(i, v => (v < 0 ? -1 : 1)),
	},
	acos: {
		calc: Math.acos,
		grad: (i, o) => Matrix.map(i, v => -1 / (Math.sqrt(1 - v ** 2) + 1.0e-4)),
	},
	acosh: {
		calc: Math.acosh,
		grad: (i, o) => Matrix.map(i, v => 1 / (Math.sqrt(v ** 2 - 1) + 1.0e-4)),
	},
	asin: {
		calc: Math.asin,
		grad: (i, o) => Matrix.map(i, v => 1 / (Math.sqrt(1 - v ** 2) + 1.0e-4)),
	...
	asinh: {
		calc: Math.asinh,
		grad: (i, o) => Matrix.map(i, v => 1 / Math.sqrt(1 + v ** 2)),
	},
	atan: {
		calc: Math.atan,
		grad: (i, o) => Matrix.map(i, v => 1 / (1 + v ** 2)),
	},
	atanh: {
		calc: Math.atanh,
		grad: (i, o) => Matrix.map(i, v => 1 / (1 - v ** 2)),
	...
	cos: {
		calc: Math.cos,
		grad: (i, o) => Matrix.map(i, v => -Math.sin(v)),
	},
	cosh: {
		calc: Math.cosh,
		grad: (i, o) => Matrix.map(i, Math.sinh),
	},
	exp: {
		calc: Math.exp,
		grad: (i, o) => o.copy(),
	...
	relu: {
		calc: v => (v > 0 ? v : 0),
		grad: (i, o) => Matrix.map(o, v => (v > 0 ? 1 : 0)),
	},
	sin: {
		calc: Math.sin,
		grad: (i, o) => Matrix.map(i, Math.cos),
	},
	sinh: {
		calc: Math.sinh,
		grad: (i, o) => Matrix.map(i, Math.cosh),
	},
	softsign: {
		calc: v => v / (1 + Math.abs(v)),
		grad: (i, o) => Matrix.map(i, v => 1 / (1 + Math.abs(v)) ** 2),
	},
	sqrt: {
		calc: Math.sqrt,
		grad: (i, o) => Matrix.map(o, v => 1 / (2 * v)),
	},
	square: {
		calc: v => v ** 2,
		grad: (i, o) => Matrix.mult(i, 2),
	},
	tan: {
		calc: Math.tan,
		grad: (i, o) => Matrix.map(i, v => 1 / Math.cos(v) ** 2),
	},
	tanh: {
		calc: Math.tanh,
		grad: (i, o) => Matrix.map(o, v => 1 - v ** 2),
	},
}
