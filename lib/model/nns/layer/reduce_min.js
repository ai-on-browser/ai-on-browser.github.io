 ...
 * Reduce min layer
 */
export default class ReduceMinLayer extends Layer {
	/**
	...
	calc(x) {
		...
		if (!this._keepdims && a.includes(-1)) {
			this._o = x.reduce((s, v) => Math.min(s, v), Infinity)
			return new Tensor([], this._o)
		}
		if (!this._keepdims && x instanceof Matrix) {
			x = Tensor.fromArray(x)
		}
		this._o = x.reduce((s, v) => Math.min(s, v), Infinity, a, true)
		if (!this._keepdims) {
			const o = this._o.copy()
			o.reshape(...o.sizes.filter((_, k) => !a.includes(k)))
			return o
		}
		return this._o
	}
