...
export default class GraphConvolutionalLayer extends Layer {
	...
	calc(x) {
		...
		if (!this._w) {
			this._w = Matrix.randn(x.value[0].nodes[0].sizes.at(-1), this._out_size)
		}
		if (!this._b) {
		...
		for (let k = 0; k < x.length; k++) {
			...
			for (let i = 0; i < graph.order; i++) {
				const v = x.value[k].getNode(i).copy()
	...
	grad(bo) {
		...
		for (let k = 0; k < bo.length; k++) {
			for (let i = 0; i < bo.value[k].order; i++) {
				...
				if (this._aggregate === 'mean') {
					bik.div(adj.length + 1)
				}
				this._bi.value[k].nodes[i].broadcastOperate(bik, (a, b) => a + b)
				for (let j = 0; j < adj.length; j++) {
					this._bi.value[k].nodes[adj[j]].broadcastOperate(bik, (a, b) => a + b)
				}
			}
		}
		this._dw.div(count)
...
GraphConvolutionalLayer.registLayer('graph_conv')
