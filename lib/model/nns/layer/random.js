...
export default class RandomLayer extends Layer {
	...
	calc() {
		if (typeof this._size === 'string') {
			const sizes = this.graph.getNode(this._size).outputValue.value
			if (sizes.length === 2) {
		...
		if (Array.isArray(this._size)) {
			if (this._size.length === 2) {
				return Matrix.randn(this._size, this._mean, this._variance)
			}
			return Tensor.randn(this._size, this._mean, this._variance)
		}
		return Tensor.randn([this._size], this._mean, this._variance)
	}
