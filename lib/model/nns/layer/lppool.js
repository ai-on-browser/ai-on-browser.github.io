 ...
 * Lp pool layer
 */
export default class LpPoolLayer extends Layer {
	/**
	 * @param {object} config object
	 * @param {number} [config.p] p
	 ...
	 */
	constructor({ p = 2, kernel, stride = null, padding = null, channel_dim = -1, ...rest }) {
		super(rest)
		this._p = p
		this._kernel = kernel
				...
				do {
					...
					do {
						...
						if (p.every((v, i) => 0 <= v && v < x.sizes[i + koff])) {
							sumval += Math.abs(x.at(this._index(i, c, p))) ** this._p
						}
					...
					this._sp.set(this._index(i, c, idx), sumval)
		...
		for (let i = 0; i < this._i.sizes[0]; i++) {
			for (let c = 0; c < channels; c++) {
				...
				do {
					...
					for (const p of ps) {
						const iidx = this._index(i, c, p)
						const oidx = this._index(i, c, idx)
						this._bi.operateAt(
							iidx,
							v =>
								v +
								this._bo.at(oidx) *
									this._sp.at(oidx) ** (1 / this._p - 1) *
									this._i.at(iidx) ** (this._p - 1) *
									Math.sign(this._i.at(iidx)) ** this._p
						)
					}
					for (let k = 0; k < idx.length; k++) {
						idx[k]++
