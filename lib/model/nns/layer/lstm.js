...
export default class LSTMLayer extends Layer {
	 ...
	 * @param {number[][] | Matrix} [config.w_in] Weight of 'in' from input to sequence
	 * @param {number[][] | Matrix} [config.w_for] Weight of 'for' from input to sequence
	 * @param {number[][] | Matrix} [config.w_out] Weight of 'out' from input to sequence
	 * @param {number[][] | Matrix} [config.r_z] Weight of z from sequence to sequence
	 * @param {number[][] | Matrix} [config.r_in] Weight of 'in' from sequence to sequence
	 * @param {number[][] | Matrix} [config.r_for] Weight of 'for' from sequence to sequence
	 * @param {number[][] | Matrix} [config.r_out] Weight of 'out' from sequence to sequence
	 * @param {number[][] | Matrix} [config.p_in] p_in
	 * @param {number[][] | Matrix} [config.p_for] p_for
	 * @param {number[][] | Matrix} [config.p_out] p_out
	 * @param {number[][] | Matrix} [config.b_z] Bias of z of output
	 * @param {number[][] | Matrix} [config.b_in] Bias of 'in' of output
	 * @param {number[][] | Matrix} [config.b_for] Bias of 'for' of output
	 * @param {number[][] | Matrix} [config.b_out] Bias of 'out' of output
	 */
	constructor({
		size,
...
class LSTMUnitLayer extends Layer {
	...
	calc(x, k) {
		...
		ib.add(Matrix.mult(this._p_in, pre_c))
		ib.add(this._b_in)
		this._ib[k] = ib
		...
		fb.add(Matrix.mult(this._p_for, pre_c))
		fb.add(this._b_for)
		this._fb[k] = fb
		...
		ob.add(Matrix.mult(this._p_out, this._c[k]))
		ob.add(this._b_out)
		this._ob[k] = ob
	...
	_grad_bptt(bo, t) {
		...
		this._dc[t].add(Matrix.mult(this._p_out, this._do[t]))
		if (t < s - 1) {
			this._dc[t].add(Matrix.mult(this._p_in, this._di[t + 1]))
			this._dc[t].add(Matrix.mult(this._p_for, this._df[t + 1]))
