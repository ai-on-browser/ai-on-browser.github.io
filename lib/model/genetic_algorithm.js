 ...
 * @property {function(): GeneticModel} mutation
 * @property {function(GeneticModel): GeneticModel} mix
 * @property {function(): number} score
 */
/**
 * Genetic algorithm
 */
...
class GeneticAlgorithmAgent {
	...
	run(env) {
		...
		while (c++ < this._max_epoch) {
			...
			if (action[0] == null) {
				console.log(c, cur_state, action, this)
			}
