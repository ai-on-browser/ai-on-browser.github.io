...
class AgglomerativeClustering {
	 ...
	 * @returns {{point?: number[], index?: number, distance?: number, distances?: number[], size: number, children?: *[]}[]} Cluster nodes
	 */
	getClusters(number) {
	...
	predict(k) {
		...
		for (let i = 0; i < clusters.length; i++) {
			const leafs = this._leafs(clusters[i])
	 ...
	 *
	 * @abstract
	 * @param {{point?: number[], index?: number, distance?: number, distances?: number[], size: number, children?: *[]}} c1 Node
	 * @param {{point?: number[], index?: number, distance?: number, distances?: number[], size: number, children?: *[]}} c2 Node
	...
	_leafs(tree) {
		let leafs = [tree]
		while (leafs.some(l => l.children)) {
			const newLeafs = []
			for (let i = 0; i < leafs.length; i++) {
				if (!leafs[i].children) {
					newLeafs.push(leafs[i])
				} else {
					newLeafs.push(...leafs[i].children)
				}
			}
			leafs = newLeafs
		}
...
export class CompleteLinkageAgglomerativeClustering extends AgglomerativeClustering {
	 ...
	 */
	distance(c1, c2) {
		let f1 = this._leafs(c1)
		let f2 = this._leafs(c2)
...
export class WardsAgglomerativeClustering extends AgglomerativeClustering {
	 ...
	 */
	distance(c1, c2) {
		let f1 = this._leafs(c1).map(f => f.point)
		let f2 = this._leafs(c2).map(f => f.point)
		let fs = f1.concat(f2)
...
export class MedianAgglomerativeClustering extends AgglomerativeClustering {
	 ...
	 */
	distance(c1, c2) {
		let m1 = this._mean(this._leafs(c1).map(f => f.point))
		let m2 = this._mean(this._leafs(c2).map(f => f.point))
