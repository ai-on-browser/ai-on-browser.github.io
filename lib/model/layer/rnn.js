...
export default class RNNLayer extends Layer {
	...
	_grad_bptt(bo) {
		...
		if (this._recurrent_activation_func) {
			this._recurrent_activation_func.calc(this._u[s - 1])
			this._bi[s - 1] = this._recurrent_activation_func.grad(this._bi[s - 1])
		}
		for (let i = s - 2; i >= 0; i--) {
			...
			if (this._recurrent_activation_func) {
				this._recurrent_activation_func.calc(this._u[i])
				this._bi[i] = this._recurrent_activation_func.grad(this._bi[i])
			}
		}
