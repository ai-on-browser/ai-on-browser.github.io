...
export default class LSTMLayer extends Layer {
	constructor({ size, return_sequences = false, ...rest }) {
		...
		this._p_in = Matrix.randn(1, this._size)
		this._p_for = Matrix.randn(1, this._size)
		this._p_out = Matrix.randn(1, this._size)
	...
	_grad_bptt(bo) {
		...
		const bi = []
		for (let t = s - 1; t >= 0; t--) {
			this._dy[t] = this._bo[t] || Matrix.zeros(...this._dy[t + 1].sizes)
			if (t < s - 1) {
