...
export class ROMMA {
	 ...
	 */
	update(x, y) {
		const m = this._w.tDot(x).toScaler() + this._w0
		if (!this._mistake(m, y)) return

		const wnorm = this._w.norm() ** 2 + this._w0 ** 2
		if (wnorm === 0) {
			this._w = Matrix.mult(x, y)
			this._w0 = y
			return
		}
		const xwnorm = (x.norm() ** 2 + 1) * wnorm

		const c = (xwnorm - y * m) / (xwnorm - m ** 2)
		const d = (wnorm * (y - m)) / (xwnorm - m ** 2)

		const dw = Matrix.mult(this._w, c)
		dw.add(Matrix.mult(x, d))
		this._w = dw
