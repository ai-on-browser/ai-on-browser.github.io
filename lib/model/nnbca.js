/**
 *  Natural Neighborhood Based Classification Algorithm
 */
export default class NNBCA {
	// Natural Neighborhood-Based Classification Algorithm Without Parameter k
	// https://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=8400443
	/**
	 * @param {'euclid' | 'manhattan' | 'chebyshev' | 'minkowski'} [metric=euclid] Metric name
	 */
	constructor(metric = 'euclid') {
		this._metric = metric
		switch (this._metric) {
			case 'euclid':
				this._d = (a, b) => Math.sqrt(a.reduce((s, v, i) => s + (v - b[i]) ** 2, 0))
				break
			case 'manhattan':
				this._d = (a, b) => a.reduce((s, v, i) => s + Math.abs(v - b[i]), 0)
				break
			case 'chebyshev':
				this._d = (a, b) => Math.max(...a.map((v, i) => Math.abs(v - b[i])))
				break
			case 'minkowski':
				this._dp = 2
				this._d = (a, b) =>
					Math.pow(
						a.reduce((s, v, i) => s + (v - b[i]) ** this._dp, 0),
						1 / this._dp
					)
				break
		}
	}

	/**
	 * Add datas.
	 *
	 * @param {Array<Array<number>>} datas Training data
	 * @param {*[]} targets Target values
	 */
	fit(datas, targets) {
		this._x = datas
		this._c = targets
		this._classes = [...new Set(this._c)]
		this._y = targets.map(v => this._classes.indexOf(v))

		const n = this._x.length

		this._distances = Array.from({ length: n }, () => [])
		for (let i = 0; i < n; i++) {
			this._distances[i][i] = { i, d: 0 }
			for (let j = i + 1; j < n; j++) {
				const dij = this._d(this._x[i], this._x[j])
				this._distances[i][j] = { i: j, d: dij }
				this._distances[j][i] = { i, d: dij }
			}
			this._distances[i].sort((a, b) => a.d - b.d)
		}

		this._g = Array.from({ length: n }, () => [])
		this._r = 1
		const knn = Array.from({ length: n }, () => [])
		let lastCnt = [-1, 0]
		while (true) {
			for (let i = 0; i < n; i++) {
				const nn = this._distances[i][this._r + 1].i
				knn[i].push(nn)
				if (knn[nn].indexOf(i) >= 0 && !this._g[i][nn]) {
					this._g[i].push(nn)
					this._g[nn].push(i)
				}
			}
			const c = this._g.reduce((s, v) => s + (v.length === 0 ? 1 : 0), 0)
			if (lastCnt[0] !== c) {
				lastCnt = [c, 1]
			} else {
				lastCnt[1]++
			}

			if (c === 0 || (this._r > 2 && lastCnt[1] >= Math.sqrt(this._r - lastCnt[1]))) {
				break
			}
			this._r++
		}

		this._t = []
		for (let k = 0; k < this._classes.length; k++) {
			this._t[k] = { s: 0, c: 0 }
			for (let i = 0; i < this._x.length; i++) {
				if (this._y[i] !== k) {
					continue
				}
				this._t[k].c++
				if (this._g[i].length === 0) {
					continue
				}
				this._t[k].s += this._g[i].reduce((s, v) => (s + this._y[v] === k ? 1 : 0), 0) / this._g[i].length
			}
		}
	}

	/**
	 * Returns predicted categories.
	 *
	 * @param {Array<Array<number>>} datas Sample data
	 * @returns {*[]} Predicted values
	 */
	predict(datas) {
		const p = []
		for (let i = 0; i < datas.length; i++) {
			const d = this._x.map((v, k) => ({ i: k, d: this._d(v, datas[i]) }))
			d.sort((a, b) => a.d - b.d)

			const nn = []
			for (let k = 0; k < this._r; k++) {
				if (d[k].d < this._distances[d[k].i][this._r + 1].d) {
					nn.push(d[k].i)
				}
			}
			if (nn.length === 0) {
				p[i] = -1
				continue
			}

			let maxt = 0
			let maxs = -1
			for (let s = 0; s < this._classes.length; s++) {
				let theta = 0
				for (let k = 0; k < this._classes.length; k++) {
					if (s === k) {
						const r =
							nn.length === 0 ? 0 : nn.reduce((s, v) => s + (this._y[v] === k ? 1 : 0), 0) / nn.length
						theta += (this._t[k].s + r) / (this._t[k].c + 1)
					} else if (this._t[k].c > 0) {
						theta += this._t[k].s / this._t[k].c
					}
				}
				if (maxt < theta) {
					maxt = theta
					maxs = s
				}
			}
			p[i] = this._classes[maxs]
		}
		return p
	}
}
