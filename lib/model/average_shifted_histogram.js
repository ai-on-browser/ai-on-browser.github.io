...
export default class AverageShiftedHistogram {
	...
	fit(datas) {
		...
		do {
			...
			const hist = new Histogram({ domain: bins, size: this._step * binSize })
			const histdata = hist.fit(datas)
			...
			do {
				...
				for (let i = 0; i < d - 1; i++) {
					hd = hd[idx[i]]
					hi.push(Math.floor((idx[i] + k[i]) / this._step))
				}
				hi.push(Math.floor((idx[d - 1] + k[d - 1]) / this._step))
				hd[idx[d - 1]] += histdata[hist._to_pos(hi)]
