...
export default class UniversalSetNaiveBayes {
	...
	fit(datas, labels) {
		...
		for (let k = 0; k < this._labels.length; k++) {
			...
			const xc = datas.filter((_, i) => labels[i] !== this._labels[k])
			...
			this._ratec[k] = xc.length / datas.length
		}
	}
	...
	predict(data) {
		...
		for (let i = 0; i < this._labels.length; i++) {
			...
			const pc = this._pc[i]._data_prob(data)
			ps.push(p.map((v, j) => (this._rate[i] * v) / (this._ratec[i] * pc[j])))
		}
		return data.map((_, n) => {
			let max_p = 0
			let max_c = -1
