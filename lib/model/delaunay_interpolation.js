...
class Triangle {
	...
	contains(p) {
		...
		const o1 = outer(p, this.p[0], this.p[1])
		const o2 = outer(p, this.p[1], this.p[2])
		const o3 = outer(p, this.p[2], this.p[0])
	...
	contains_circle(p) {
		return (p.x - this._c[0]) ** 2 + (p.y - this._c[1]) ** 2 < this._r2
	}
...
export default class DelaunayInterpolation {
	...
	fit(x, y) {
		...
		const rootPoints = [
			new Point([min[0] - (max[1] - min[1]), min[1]], null),
			new Point([max[0] + (max[1] - min[1]), min[1]], null),
			new Point([(min[0] + max[0]) / 2, max[1] + (max[0] - min[0]) / 2], null),
		]
