...
export class KMeans extends KMeansBase {
	...
	add(datas) {
		...
		const domain = Array.from(datas[0], () => [Infinity, -Infinity])
		for (let i = 0; i < datas.length; i++) {
			for (let j = 0; j < datas[i].length; j++) {
				domain[j][0] = Math.min(domain[j][0], datas[i][j])
				domain[j][1] = Math.max(domain[j][1], datas[i][j])
			}
		}
		const cpoint = domain.map(d => Math.random() * (d[1] - d[0]) + d[0])
		this._centroids.push(cpoint)
		return cpoint
	}
	...
	_move(centroids, datas) {
		...
		for (let k = 0; k < centroids.length; k++) {
			c[k] = Array(centroids[k].length).fill(0)
		}
		const cnt = Array(centroids.length).fill(0)
		for (let k = 0; k < centroids.length; k++) {
			for (let i = 0; i < datas.length; i++) {
				for (let j = 0; j < datas[i].length; j++) {
					c[pred[i]][j] += datas[i][j]
				}
				cnt[pred[i]]++
			}
		}
		for (let k = 0; k < centroids.length; k++) {
			c[k] = c[k].map(v => v / cnt[k])
		}
		return c
	}
}
...
export class KMeanspp extends KMeans {
	...
	add(datas) {
		let cpoint = null
		if (this._centroids.length === 0) {
			cpoint = datas[Math.floor(Math.random() * datas.length)].concat()
		} else {
			let s = 0
			const d = []
			for (let i = 0; i < datas.length; i++) {
				d[i] =
					Math.min.apply(
						null,
						this._centroids.map(c => this._distance(datas[i], c))
					) ** 2
				s += d[i]
			}
			let r = Math.random() * s
...
export class KMedoids extends KMeans {
	...
	_move(centroids, datas) {
		...
		return centroids.map((c, k) => {
			const catpoints = datas.filter((_, i) => pred[i] === k)
			if (catpoints.length > 0) {
				const sd = Array(catpoints.length).fill(0)
				for (let i = 0; i < catpoints.length; i++) {
					for (let j = 0; j < i; j++) {
						const d = this._distance(catpoints[i], catpoints[j])
						sd[i] += d
						sd[j] += d
					}
				}
...
export class SemiSupervisedKMeansModel extends KMeansBase {
	...
	init(datas, labels) {
		...
		for (let k = 0; k < this._classes.length; k++) {
			const labeledDatas = datas.filter((_, i) => labels[i] === this._classes[k])
	...
	fit(datas, labels) {
		...
		this._centroids = this._centroids.map((_, k) => {
			const catpoints = datas.filter((_, i) => pred[i] === this._classes[k])
