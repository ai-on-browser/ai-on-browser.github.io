...
class GaussianKernel {
	...
	_calc(x0, x1) {
		const s = Matrix.sub(x0, x1).reduce((acc, v) => acc + v * v, 0)
		return this._a * Math.exp(-s / (2 * this._b))
	}

	_grad(x0, x1, k) {
		const g = Matrix.sub(x0, x1)
	...
	update(x, k, G) {
		...
		for (let i = 0; i < n; i++) {
			const xi = x.row(i)
			for (let j = 0; j < n; j++) {
				const di = Matrix.sub(xi, x.row(j))
				di.map(v => v ** 2)
				const s = -(this._a * di.sum()) / (2 * this._b ** 2)
				d.set(i, j, s * k.at(i, j))
			}
		}
		const dsig = Matrix.mult(G, d.t).sum()
...
export default class GPLVM {
	 ...
	 * @param {number} [ez=1.0] Learning rate for z
	 * @param {number} [ea=0.005] Learning rate for alpha
	 * @param {number} [ep=0.2] Learning rate for kernel
	 * @param {'gaussian' | { name: 'gaussian', a?: number, b?: number}} [kernel=gaussian] Kernel name
	 */
	constructor(rd, alpha, ez = 1, ea = 0.005, ep = 0.2, kernel = 'gaussian') {
		this._rd = rd
		this._alpha = alpha
		if (typeof kernel === 'string') {
			kernel = { name: kernel }
		}
		this._kernel = new GaussianKernel(kernel.a, kernel.b, ep)
		this._ez = ez
		this._ea = ea
	}
	...
	fit() {
		...
		const K = Matrix.add(ker, Matrix.eye(n, n, this._alpha))
		const Kinv = K.inv()
		const G = Kinv.dot(this._s).dot(Kinv)
		G.sub(Matrix.mult(Kinv, this._x.cols))
		G.div(2)
	...
	reconstruct(z) {
		...
		const Ka = Matrix.add(K, Matrix.eye(n, n, this._alpha))
