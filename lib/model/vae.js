...
export default class VAE {
	...
	constructor(in_size, noise_dim, enc_layers, dec_layers, optimizer, class_size, type) {
		...
		this._decodeNet = new NeuralNetwork(decodeLayers, null, optimizer)
		aeLayers.push(
		...
		this._aeNet = new NeuralNetwork(aeLayers, null, optimizer)
	}
	 ...
	 * @param {Array<Array<number>>} y
	 * @param {string[]} out
	 * @returns {Array<Array<number>> | Record<string, Array<Array<number>>>}
	 */
	predict(x, y, out) {
		const pred = this._aeNet.calc({ enc_in: x, cond: y }, null, out)
		if (out) {
			for (const k of Object.keys(pred)) {
				pred[k] = pred[k].toArray()
			}
			return pred
		}
		return pred.toArray()
