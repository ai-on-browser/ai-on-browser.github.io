...
import Layer, { LossLayer } from './nns/layer/base.js'
...
export default class NeuralNetwork {
	...
	static async fromONNX(buffer) {
		if (!ONNXImporter) {
			ONNXImporter = (await import('./nns/onnx/onnx_importer.js')).default
		}
		return ONNXImporter.load(buffer)
	}
	...
	calc(x, t, out, options = {}) {
		...
		if (!out && !t) {
			for (let i = 0; i < this._graph.nodes.length; i++) {
				if (this._graph.nodes[i].layer instanceof OutputLayer) {
					this._graph.calc([i])
