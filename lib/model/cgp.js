...
class Program {
	constructor(funcs, n, m, r, c, levels_back = c) {
		...
		this._const_nodes = [1]
		this._n = n
		this._n_inputs = n + this._const_nodes.length
		this._n_outputs = m
		this._rows = r
		this._cols = c
	...
	mutate(rate = 0.03) {
		const prog = new Program(this._funcs, this._n, this._n_outputs, this._rows, this._cols, this._levels_back)
	...
	evaluate(x) {
		...
		for (let i = 0; i < this._const_nodes.length; i++) {
			values[i + this._n] = Array(x.length).fill(this._const_nodes[i])
		}
	...
	toString() {
		this._init()
		const strs = Array.from({ length: this._n }, (_, i) => `x[${i}]`)
		for (let i = 0; i < this._const_nodes.length; i++) {
			strs[i + this._n] = this._const_nodes[i]
		}
		for (let i = 0; i < this._rows * this._cols; i++) {
			if (!this._active[i]) {
				continue
...
export default class CartesianGeneticProgramming {
	...
	/**
	 * Fit model.
	 * @returns {number} current best loss
	 */
	fit() {
