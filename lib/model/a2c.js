...
export default class A2CAgent {
	...
	constructor(env, resolution, procs, layers, optimizer) {
		...
		for (let i = 0; i < this._procs; i++) {
			this._envs[i] = this._env.clone()
			this._states[i] = this._envs[i].reset()
		}
	}
	...
	update(done, learning_rate, batch) {
		const actions = Array.from({ length: this._procs }, () => [])
		const states = Array.from({ length: this._procs }, () => [])
		const next_states = Array.from({ length: this._procs }, () => [])
		const rewards = Array.from({ length: this._procs }, () => [])
		const dones = Array.from({ length: this._procs }, () => [])
