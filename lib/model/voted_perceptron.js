...
export default class VotedPerceptron {
	...
	fit() {
		for (let i = 0; i < this._x.length; i++) {
			if (this._v.length === 0) {
				this._v.push(this._x[i].map(v => this._r * this._y[i] * v))
				this._b.push(this._r * this._y[i])
				this._c.push(1)
				continue
			}
			...
			if (yh === this._y[i]) {
				...
				this._v.push(this._x[i].map((v, j) => vk[j] + this._r * this._y[i] * v))
				this._b.push(this._b[this._b.length - 1] + this._r * this._y[i])
				this._c.push(1)
			}
		}
	}
