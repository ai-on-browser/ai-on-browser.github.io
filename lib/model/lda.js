...
export class LinearDiscriminant {
	...
	fit() {
		...
		const x0 = Matrix.fromArray(this._x.filter((_, i) => this._y[i] === 1))
		const x1 = Matrix.fromArray(this._x.filter((_, i) => this._y[i] === -1))
		const m0 = x0.mean(0)
		const m1 = x1.mean(0)
		const s = x.cov()
		const sinv = s.inv()

		this._w = Matrix.sub(m0, m1).dot(sinv).t
		this._w0 =
...
export class FishersLinearDiscriminant {
	...
	fit() {
		...
		const Sw = x0.tDot(x0)
		Sw.add(x1.tDot(x1))
		this._w = Matrix.sub(m0, m1).dot(Sw.inv()).t
...
export class MulticlassLinearDiscriminant {
	...
	fit(x, y) {
		...
		for (let i = 0; i < this._c.length; i++) {
			let xi = x.filter((_, k) => y[k] === this._c[i])
	...
	predict(data) {
		...
		for (let i = 0; i < this._c.length; i++) {
			const xi = Matrix.sub(x, this._m[i])
...
export class LinearDiscriminantAnalysis {
	...
	predict(x, t) {
		...
		const cov = w.solve(b)
		const ev = cov.eigenVectors()
		if (this._rd > 0 && this._rd < ev.cols) {
			ev.resize(ev.rows, this._rd)
		}
