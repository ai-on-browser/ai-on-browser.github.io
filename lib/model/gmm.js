...
export class GMM {
	...
	predict(data) {
		...
		return data.map(v => {
			...
			let max_c = -1
			for (let i = 0; i < this._k; i++) {
				const v = this._gaussian(x, this._m[i], this._s[i])
				if (v > max_p) {
					max_p = v
					max_c = i
				}
	...
	fit(datas) {
		...
		for (let i = 0; i < this._k; i++) {
			...
			for (let j = 0; j < n; j++) {
				new_mi.add(Matrix.mult(x[j], g[j][i]))
			}
			new_mi.div(N[i])
			this._m[i] = new_mi
...
export class GMR extends GMM {
	...
	predict(x) {
		...
		for (let k = 0; k < this._k; k++) {
			const c = Matrix.sub(x, this._mx[k].t).dot(this._sxx[k].inv()).dot(this._sxy[k])
			c.add(this._my[k])
			c.mult(w.col(k))
			ys.add(c)
		}
