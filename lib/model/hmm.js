...
export class ContinuousHMM extends HMMBase {
	...
	fit(x, scaled = false) {
		if (!this._d) {
			this._d = Array.isArray(x[0][0]) ? x[0][0].length : 1
			for (let i = 0; i < this._n; i++) {
...
export class HMMClassifier {
	 ...
	 * @param {*} cls HMM class
	 */
	constructor(n, cls = ContinuousHMM) {
		this._n = n
		this._classes = null
		this._models = []
		this._modelcls = cls
	...
	fit(x, y, scaled = false) {
		if (!this._classes) {
			...
			for (let i = 0; i < this._classes.length; i++) {
				const m = new this._modelcls(this._n)
				this._models.push(m)
			}
		}
	 ...
	 *
	 * @param {Array<Array<number>> | Array<Array<Array<number>>>} x Sample data
