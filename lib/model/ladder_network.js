...
export default class LadderNetwork {
	...
	_build() {
		...
		this._layers = [
			{ type: 'input', name: 'x' },
			{ type: 'identity', name: 'z_0' },
			{ type: 'shape', name: 'x_shape' },
			{ type: 'random', size: 'x_shape', variance: this._noise_std[0] ** 2, name: 'noize' },
		...
		for (let i = 0; i < hidden_sizes.length; i++) {
			...
			this._layers.push(
				...
				{ type: 'shape', name: `ztbn_${k}_shape` },
				{ type: 'random', size: `ztbn_${k}_shape`, variance: this._noise_std[k] ** 2, name: `noize_${k}` },
		...
		this._layers.push(
			...
			{ type: 'identity', input: 'reduced[1]' }
		)

		for (let k = hidden_sizes.length; k >= 0; k--) {
			const size = k === 0 ? 'x_shape' : hidden_sizes[k - 1]
			...
			this._layers.push(
				{ type: 'shape', input: `mean_${k}`, name: `mean_${k}_shape` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a1_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a2_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a3_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a4_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a5_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a6_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a7_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a8_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a9_${k}` },
				{ type: 'variable', size: `mean_${k}_shape`, name: `a0_${k}` },
	...
	fit(train_x, train_y, iteration, rate, batch) {
		...
		let lLoss = null
		let uLoss = null
		const labeled_x = train_x.filter((_, i) => train_y[i] != null)
		...
		const unlabeled_x = train_x.filter((_, i) => train_y[i] == null)
