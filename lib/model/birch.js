...
class CFTree {
	...
	_separate() {
		...
		const model = new KMeans(d)
		model.fit()
		const p = model.predict()
		if (this.isLeaf()) {
			if (this.isRoot()) {
				const c1 = new CFTree(this._b, this._t, this._l)
				c1._datas = d.filter((_, i) => p[i] === 0)
				c1._parent = this
				const c2 = new CFTree(this._b, this._t, this._l)
				c2._datas = d.filter((_, i) => p[i] === 1)
				c2._parent = this
				this._children = [c1, c2]
				this._datas = null
			} else {
				const new_c = new CFTree(this._b, this._t, this._l)
				new_c._datas = d.filter((_, i) => p[i] === 1)
				new_c._parent = this._parent
				this._parent._children.push(new_c)
				this._datas = d.filter((_, i) => p[i] === 0)
			}
		} else {
			if (this.isRoot()) {
				const c1 = new CFTree(this._b, this._t, this._l)
				c1._children = this._children.filter((_, i) => p[i] === 0)
				c1._children.forEach(c => {
					c._parent = c1
				})
				c1._parent = this
				const c2 = new CFTree(this._b, this._t, this._l)
				c2._children = this._children.filter((_, i) => p[i] === 1)
				c2._children.forEach(c => {
					c._parent = c2
				})
				c2._parent = this
				this._children = [c1, c2]
				...
				const new_c = new CFTree(this._b, this._t, this._l)
				new_c._children = this._children.filter((_, i) => p[i] === 1)
				new_c._children.forEach(c => {
					c._parent = new_c
				})
				new_c._parent = this._parent
				this._parent._children.push(new_c)
				this._children = this._children.filter((_, i) => p[i] === 0)
			}
		}
	}
}
