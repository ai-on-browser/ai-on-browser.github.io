...
export default class Tensor {
	...
	*[Symbol.iterator]() {
	...
	toMatrix() {
		if (this.dimension !== 2) {
			throw new MatrixException('Only 2D tensor can convert to matrix.', this)
		}
	...
	toScaler() {
		if (this._value.length !== 1) {
			throw new MatrixException('The tensor cannot convert to scaler.', this)
		}
	...
	at(...i) {
		...
		if (i.length !== this.dimension) {
			throw new MatrixException(`Number of dimensions is ${this.dimension} but got ${i.length}.`, this)
		}
	...
	reshape(...sizes) {
		...
		if (negidx >= 0) {
			...
			if (this.length % rest !== 0) {
				throw new MatrixException(`Length is different. This: ${this.length}, Sizes: ${sizes}.`, this)
			}
	...
	concat(t, axis = 0) {
		if (this.dimension !== t.dimension) {
			throw new MatrixException(`Size is different. This: ${this.dimension}, Other: ${t.dimension}.`, this)
		}
		if (axis < 0 || this.dimension <= axis) {
			throw new MatrixException(`Axis is out of bounds. Axis: ${axis}, Dimension: ${this.dimension}.`, this)
		}
		for (let d = 0; d < this.dimension; d++) {
			if (axis !== d && this._size[d] !== t._size[d]) {
				throw new MatrixException(
					`Size is different at dimension ${d}. This: ${this._size[d]}, Other: ${t._size[d]}.`,
					this
				)
			}
		}
	 ...
	 * @returns {Tensor<U> | U} Reduced tensor or value
	 */
