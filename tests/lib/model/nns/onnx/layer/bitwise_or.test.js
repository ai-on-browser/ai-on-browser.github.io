...
describe('export', () => {
	test.each([2, 3, 4])('array input %p', length => {
		...
		const info = bitwise_or.export(model, { type: 'bitwise_or', input }, types)
		...
		for (let i = 1; i < input.length; i++) {
			expect(nodes[1].getOpType()).toBe('BitwiseOr')
		}
	})

	test('array input 1', () => {
		const model = ONNXExporter.createONNXModel()
		const info = bitwise_or.export(model, { type: 'bitwise_or', input: ['x'] })
	...
	test('string input', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => bitwise_or.export(model, { type: 'bitwise_or', input: 'x' })).toThrow("Invalid attribute 'input'")
	})
})
...
describe('runtime', () => {
	...
	test.each([
		...
		const yten = out._bitwise_or
		expect(yten.dims).toEqual([100, 3])
		const y = await yten.getData(true)

		const t = Layer.fromObject({ type: 'bitwise_or' }).calc(...x)
