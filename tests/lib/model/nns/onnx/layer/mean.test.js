...
describe('export', () => {
	test('default', () => {
		...
		const info = mean.export(model, { type: 'mean', input: 'x' }, { x: { size: [null, 5] } })
	...
	test('array input', () => {
		const model = ONNXExporter.createONNXModel()
		const info = mean.export(model, { type: 'mean', input: ['x'], axis: 0 }, { x: { size: [null, 5] } })
	...
	test('keepdims false', () => {
		const model = ONNXExporter.createONNXModel()
		const info = mean.export(model, { type: 'mean', input: 'x', keepdims: false }, { x: { size: [null, 5] } })
	...
	test('keepdims false with axis', () => {
		const model = ONNXExporter.createONNXModel()
		const info = mean.export(model, { type: 'mean', input: 'x', keepdims: false, axis: 1 }, { x: { size: [2, 5] } })
	...
	test('multi axis', () => {
		const model = ONNXExporter.createONNXModel()
		const info = mean.export(model, { type: 'mean', input: 'x', axis: [0, 1] }, { x: { size: [null, 5] } })
		expect(info).toEqual({ size: [1, 1] })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(1)
		expect(nodes[0].getOpType()).toBe('ReduceMean')
	...
	test('string axis', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => mean.export(model, { type: 'mean', input: 'x', axis: 'a' }, { x: { size: [null, 5] } })).toThrow(
...
describe('runtime', () => {
	...
	])('mean %j %j %j %j', async (param, inSize, actualSize, outSize) => {
		const buf = ONNXExporter.dump([{ type: 'input', size: inSize }, { type: 'mean', ...param }, { type: 'output' }])
		session = await ort.InferenceSession.create(buf)

		const x = Matrix.randn(actualSize)
		const xten = new ort.Tensor('float32', x.value, x.sizes)
		const out = await session.run({ _input: xten })
		const yten = out._mean
		expect(yten.dims).toEqual(outSize)
		const y = await yten.getData(true)

		const t = new MeanLayer(param).calc(x)
