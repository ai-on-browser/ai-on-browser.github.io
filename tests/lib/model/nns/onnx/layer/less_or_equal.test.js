...
describe('export', () => {
	test('array input 1', () => {
		...
		const info = less_or_equal.export(model, { type: 'less_or_equal', input: ['x'] })
	...
	test('array input 3', () => {
		const model = ONNXExporter.createONNXModel()
		const info = less_or_equal.export(model, { type: 'less_or_equal', input: ['x', 'y', 'z'] })
		expect(info).toEqual({ type: onnx.TensorProto.DataType.BOOL })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(3)
		expect(nodes[0].getOpType()).toBe('LessOrEqual')
		expect(nodes[1].getOpType()).toBe('LessOrEqual')
		expect(nodes[2].getOpType()).toBe('And')
	})

	test('array input 4', () => {
		const model = ONNXExporter.createONNXModel()
		const info = less_or_equal.export(model, { type: 'less_or_equal', input: ['x', 'y', 'z', 'a'] })
		expect(info).toEqual({ type: onnx.TensorProto.DataType.BOOL })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(5)
		expect(nodes[0].getOpType()).toBe('LessOrEqual')
		expect(nodes[1].getOpType()).toBe('LessOrEqual')
		expect(nodes[2].getOpType()).toBe('LessOrEqual')
...
describe('runtime', () => {
	...
	test.each([
		...
		const t = Layer.fromObject({ type: 'less_or_equal' }).calc(...x)
