...
import sum from '../../../../../../lib/model/nns/onnx/layer/sum.js'
...
describe('export', () => {
	test('default', () => {
		...
		const info = sum.export(model, { type: 'sum', input: 'x' }, { x: { size: [null, 5] } })
		expect(info).toEqual({ size: [1, 1] })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(1)
		expect(nodes[0].getOpType()).toBe('ReduceSum')
	...
	test('array input', () => {
		const model = ONNXExporter.createONNXModel()
		const info = sum.export(model, { type: 'sum', input: ['x'], axis: 0 }, { x: { size: [null, 5] } })
	...
	test('keepdims false', () => {
		const model = ONNXExporter.createONNXModel()
		const info = sum.export(model, { type: 'sum', input: 'x', keepdims: false }, { x: { size: [null, 5] } })
	...
	test('keepdims false with axis', () => {
		const model = ONNXExporter.createONNXModel()
		const info = sum.export(model, { type: 'sum', input: 'x', keepdims: false, axis: 1 }, { x: { size: [2, 5] } })
	...
	test('multi axis', () => {
		const model = ONNXExporter.createONNXModel()
		const info = sum.export(model, { type: 'sum', input: 'x', axis: [0, 1] }, { x: { size: [null, 5] } })
		expect(info).toEqual({ size: [1, 1] })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(1)
		expect(nodes[0].getOpType()).toBe('ReduceSum')
	...
	test('string axis', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => sum.export(model, { type: 'sum', input: 'x', axis: 'a' }, { x: { size: [null, 5] } })).toThrow(
...
describe('runtime', () => {
	...
	test.each([
		...
		const t = new SumLayer(param).calc(x)
