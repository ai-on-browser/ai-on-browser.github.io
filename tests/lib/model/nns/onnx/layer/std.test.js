...
import std from '../../../../../../lib/model/nns/onnx/layer/std.js'
...
describe('export', () => {
	test('default', () => {
		...
		const info = std.export(model, { type: 'std', input: 'x' }, { x: { size: [null, 5] } })
	...
	test('array input', () => {
		const model = ONNXExporter.createONNXModel()
		const info = std.export(model, { type: 'std', input: ['x'], axis: 0 }, { x: { size: [null, 5] } })
	...
	test('keepdims false', () => {
		const model = ONNXExporter.createONNXModel()
		const info = std.export(model, { type: 'std', input: 'x', keepdims: false }, { x: { size: [null, 5] } })
	...
	test('keepdims false with axis', () => {
		const model = ONNXExporter.createONNXModel()
		const info = std.export(model, { type: 'std', input: 'x', keepdims: false, axis: 1 }, { x: { size: [2, 5] } })
	...
	test('multi axis', () => {
		const model = ONNXExporter.createONNXModel()
		const info = std.export(model, { type: 'std', input: 'x', axis: [0, 1] }, { x: { size: [null, 5] } })
		expect(info).toEqual({ size: [1, 1] })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(5)
		expect(nodes[0].getOpType()).toBe('ReduceMean')
	...
	test('string axis', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => std.export(model, { type: 'std', input: 'x', axis: 'a' }, { x: { size: [null, 5] } })).toThrow(
...
describe('runtime', () => {
	...
	test.each([
	...
	])('std %p %p %p %p', async (param, inSize, actualSize, outSize) => {
		const buf = ONNXExporter.dump([{ type: 'input', size: inSize }, { type: 'std', ...param }, { type: 'output' }])
		...
		const t = new StdLayer(param).calc(x)
