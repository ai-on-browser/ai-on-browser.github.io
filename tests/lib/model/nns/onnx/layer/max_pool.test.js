...
describe('export', () => {
	test.each([{ input: 'x', channel_dim: -1 }, { input: ['x'] }])('last channel %p', param => {
		...
		expect(nodes[2].getOpType()).toBe('MaxPool')
	})

	test('first channel', () => {
		const model = ONNXExporter.createONNXModel()
		const info = maxPool.export(
			model,
			{ type: 'max_pool', input: 'x', channel_dim: 1 },
			{ x: { size: [null, 10, 3] } }
		)
		expect(info.size).toEqual([null, 10, null])
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(1)
		expect(nodes[0].getOpType()).toBe('MaxPool')
	})

	test('invalid channel dim', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() =>
			maxPool.export(model, { type: 'max_pool', input: ['x'], channel_dim: 0 }, { x: { size: [null, 10, 3] } })
...
describe('runtime', () => {
	...
	test.each([
		...
		const yten = out._max_pool
		expect(yten.dims).toEqual(outSize)
		const y = await yten.getData(true)

		const t = new MaxPoolLayer(param).calc(x)
