...
import and from '../../../../../../lib/model/nns/onnx/layer/and.js'
...
describe('export', () => {
	test.each([2, 3, 4])('array input %j', length => {
		...
		const info = and.export(model, { type: 'and', input }, types)
		...
		for (let i = 1; i < input.length; i++) {
			expect(nodes[1].getOpType()).toBe('And')
		}
	})

	test('array input 1', () => {
		const model = ONNXExporter.createONNXModel()
		const info = and.export(model, { type: 'and', input: ['x'] })
	...
	test('string input', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => and.export(model, { type: 'and', input: 'x' })).toThrow("Invalid attribute 'input'")
	})
})
...
describe('runtime', () => {
	...
	test.each([
		...
		const t = Layer.fromObject({ type: 'and' }).calc(...x)
