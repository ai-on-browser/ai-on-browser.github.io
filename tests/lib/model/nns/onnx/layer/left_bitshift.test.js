...
import left_bitshift from '../../../../../../lib/model/nns/onnx/layer/left_bitshift.js'
...
describe('export', () => {
	test.each([2, 3, 4])('array input %j', length => {
		...
		const info = left_bitshift.export(model, { type: 'left_bitshift', input }, types)
	...
	test('array input 1', () => {
		const model = ONNXExporter.createONNXModel()
		const info = left_bitshift.export(
			model,
			{ type: 'left_bitshift', input: ['x'] },
	...
	test('string input', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => left_bitshift.export(model, { type: 'left_bitshift', input: 'x' })).toThrow(
...
describe('runtime', () => {
	...
	test.each([
		...
		for (const i of inputNames) {
			const v = Matrix.randint(ins[i].a, 0, x.length === 0 ? 255 : 10)
			x.push(v)
			xten[i] = new ort.Tensor('float32', v.value, v.sizes)
		}
		const out = await session.run(xten)
		const yten = out._left_bitshift
		expect(yten.dims).toEqual([100, 3])
		const y = await yten.getData(true)

		const t = Layer.fromObject({ type: 'left_bitshift' }).calc(...x)
