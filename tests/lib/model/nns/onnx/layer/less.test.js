...
import less from '../../../../../../lib/model/nns/onnx/layer/less.js'
...
describe('export', () => {
	test('array input 1', () => {
		...
		const info = less.export(model, { type: 'less', input: ['x'] })
	...
	test('array input 2', () => {
		const model = ONNXExporter.createONNXModel()
		const info = less.export(model, { type: 'less', input: ['x', 'y'] })
		expect(info).toEqual({ type: onnx.TensorProto.DataType.BOOL })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(1)
		expect(nodes[0].getOpType()).toBe('Less')
	})

	test('array input 3', () => {
		const model = ONNXExporter.createONNXModel()
		const info = less.export(model, { type: 'less', input: ['x', 'y', 'z'] })
		expect(info).toEqual({ type: onnx.TensorProto.DataType.BOOL })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(3)
		expect(nodes[0].getOpType()).toBe('Less')
		expect(nodes[1].getOpType()).toBe('Less')
		expect(nodes[2].getOpType()).toBe('And')
	})

	test('array input 4', () => {
		const model = ONNXExporter.createONNXModel()
		const info = less.export(model, { type: 'less', input: ['x', 'y', 'z', 'a'] })
		expect(info).toEqual({ type: onnx.TensorProto.DataType.BOOL })
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(5)
		expect(nodes[0].getOpType()).toBe('Less')
		expect(nodes[1].getOpType()).toBe('Less')
		expect(nodes[2].getOpType()).toBe('Less')
	...
	test('string input', () => {
		const model = ONNXExporter.createONNXModel()
		expect(() => less.export(model, { type: 'less', input: 'x' })).toThrow("Invalid attribute 'input'")
	})
})
...
describe('runtime', () => {
	...
	test.each([
		...
		const yten = out._less
		expect(yten.dims).toEqual([100, 3])
		const y = await yten.getData(true)

		const t = Layer.fromObject({ type: 'less' }).calc(...x)
