...
import globalMaxPool from '../../../../../../lib/model/nns/onnx/layer/global_max_pool.js'
...
describe('export', () => {
	test.each([{ input: 'x', channel_dim: -1 }, { input: ['x'] }])('last channel %j', param => {
		...
		const info = globalMaxPool.export(model, { type: 'global_max_pool', ...param }, { x: { size: [null, 10, 3] } })
		...
		expect(nodes[2].getOpType()).toBe('GlobalMaxPool')
	})

	test('first channel', () => {
		const model = ONNXExporter.createONNXModel()
		const info = globalMaxPool.export(
			model,
			{ type: 'global_max_pool', input: 'x', channel_dim: 1 },
			{ x: { size: [null, 10, 3] } }
		)
		expect(info.size).toEqual([null, 10, 1])
		const nodes = model.getGraph().getNodeList()
		expect(nodes).toHaveLength(1)
		expect(nodes[0].getOpType()).toBe('GlobalMaxPool')
	})
	...
	test('invalid channel dim', () => {
		...
		expect(() =>
			globalMaxPool.export(
				model,
				{ type: 'global_max_pool', input: ['x'], channel_dim: 0 },
				{ x: { size: [null, 10, 3] } }
			)
...
describe('runtime', () => {
	...
	test.each([
	...
	])('global max pool %j %j %j %j', async (param, inSize, actualSize, outSize) => {
		const buf = ONNXExporter.dump([
			{ type: 'input', size: inSize },
			{ type: 'global_max_pool', ...param },
			{ type: 'output' },
		])
		...
		const yten = out._global_max_pool
		expect(yten.dims).toEqual(outSize)
		const y = await yten.getData(true)

		const t = new GlobalMaxPoolLayer(param).calc(x)
