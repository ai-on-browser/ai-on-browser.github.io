...
for name, axis, kwargs in [
    ("reducesum", [1], {}),
    ("reducesum_not_keepdims", [1], {"keepdims": 0}),
    ("reducesum_noop_with_empty_axes", [1], {"noop_with_empty_axes": 1}),
]:
    ...
    node = onnx.helper.make_node(
        "ReduceSum", inputs=["x", "a"], outputs=["y"], **kwargs
    )
...
for name, kwargs in [("reducesum_no_axis", {})]:
    node = onnx.helper.make_node("ReduceSum", inputs=["x"], outputs=["y"], **kwargs)
...
for name, axis, kwargs in [("reducesum_dummy_init", [1], {})]:
    axis_init = onnx.helper.make_tensor(
        name="a",
        data_type=onnx.TensorProto.INT64,
        dims=(len(axis),),
...
for name, axis, kwargs in [("reducesum_other_node", [1], {})]:
    ...
    node = onnx.helper.make_node(
        "ReduceSum", inputs=["x", "axis"], outputs=["y"], **kwargs
    )
