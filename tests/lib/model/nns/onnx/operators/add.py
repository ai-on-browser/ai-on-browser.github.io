...
add_node = onnx.helper.make_node("Add", inputs=["x", "const"], outputs=["y"])

graph_def = onnx.helper.make_graph(
    nodes=[const_node, add_node], name="graph", inputs=[X], outputs=[Y], initializer=[]
)
model_def = onnx.helper.make_model(graph_def, producer_name="onnx-example")
onnx.checker.check_model(model_def)

onnx.save(model_def, f"{os.path.dirname(__file__)}/add.onnx")
