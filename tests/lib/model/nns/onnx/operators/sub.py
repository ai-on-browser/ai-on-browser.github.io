...
C_init = onnx.helper.make_tensor(
    name="c", data_type=onnx.TensorProto.FLOAT, dims=(1, 3), vals=[1] * 3
)
const_node = onnx.helper.make_node(
    "Constant", inputs=[], outputs=["const"], value=C_init
)
sub_node = onnx.helper.make_node("Sub", inputs=["x", "const"], outputs=["y"])

graph_def = onnx.helper.make_graph(
    nodes=[const_node, sub_node], name="graph", inputs=[X], outputs=[Y], initializer=[]
)
model_def = onnx.helper.make_model(graph_def, producer_name="onnx-example")
onnx.checker.check_model(model_def)

onnx.save(model_def, f"{os.path.dirname(__file__)}/sub.onnx")
