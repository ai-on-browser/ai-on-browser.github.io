...
for name, kwargs in [("reducemin_no_axis", {})]:
    node = onnx.helper.make_node("ReduceMin", inputs=["x"], outputs=["y"], **kwargs)
...
for name, axis, kwargs in [("reducemin_dummy_init", [1], {})]:
    axis_init = onnx.helper.make_tensor(
        name="a",
        data_type=onnx.TensorProto.INT64,
        dims=(len(axis),),
...
for name, axis, kwargs in [("reducemin_other_node", [1], {})]:
    ...
    node = onnx.helper.make_node(
        "ReduceMin", inputs=["x", "axis"], outputs=["y"], **kwargs
    )
