...
describe('load', () => {
	...
	test('reduceprod_no_axis', async () => {
		const buf = await fs.promises.readFile(`${filepath}/reduceprod_no_axis.onnx`)
		const nodes = await ONNXImporter.load(buf)
		expect(nodes).toHaveLength(3)
		expect(nodes[1]).toEqual({ type: 'prod', input: ['x'], name: 'y', axis: -1, keepdims: true })
	})

	test('reduceprod_dummy_init', async () => {
		const buf = await fs.promises.readFile(`${filepath}/reduceprod_dummy_init.onnx`)
		const nodes = await ONNXImporter.load(buf)
		expect(nodes).toHaveLength(3)
		expect(nodes[1]).toEqual({ type: 'prod', input: ['x'], name: 'y', axis: [1], keepdims: true })
	})

	test('reduceprod_other_node', async () => {
		const buf = await fs.promises.readFile(`${filepath}/reduceprod_other_node.onnx`)
		const nodes = await ONNXImporter.load(buf)
		expect(nodes).toHaveLength(4)
		expect(nodes[1]).toEqual({ type: 'const', name: 'axis', value: [1] })
		expect(nodes[2]).toEqual({ type: 'prod', input: ['x'], name: 'y', axis: 'axis', keepdims: true })
	})
})
