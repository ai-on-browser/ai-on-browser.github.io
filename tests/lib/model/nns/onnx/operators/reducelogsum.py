...
for name, axis, kwargs in [
    ("reducelogsum", [1], {}),
    ("reducelogsum_not_keepdims", [1], {"keepdims": 0}),
    ("reducelogsum_noop_with_empty_axes", [1], {"noop_with_empty_axes": 1}),
]:
    ...
    node = onnx.helper.make_node(
        "ReduceLogSum", inputs=["x", "a"], outputs=["y"], **kwargs
    )
...
for name, kwargs in [("reducelogsum_no_axis", {})]:
    node = onnx.helper.make_node("ReduceLogSum", inputs=["x"], outputs=["y"], **kwargs)
...
for name, axis, kwargs in [("reducelogsum_dummy_init", [1], {})]:
    axis_init = onnx.helper.make_tensor(
        name="a",
        data_type=onnx.TensorProto.INT64,
        dims=(len(axis),),
...
for name, axis, kwargs in [("reducelogsum_other_node", [1], {})]:
    ...
    node = onnx.helper.make_node(
        "ReduceLogSum", inputs=["x", "axis"], outputs=["y"], **kwargs
    )
