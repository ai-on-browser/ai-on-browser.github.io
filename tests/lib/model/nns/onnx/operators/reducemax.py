...
for name, axis, kwargs in [
    ("reducemax", [1], {}),
    ("reducemax_not_keepdims", [1], {"keepdims": 0}),
    ("reducemax_noop_with_empty_axes", [1], {"noop_with_empty_axes": 1}),
]:
    ...
    node = onnx.helper.make_node(
        "ReduceMax", inputs=["x", "a"], outputs=["y"], **kwargs
    )
...
for name, kwargs in [("reducemax_no_axis", {})]:
    node = onnx.helper.make_node("ReduceMax", inputs=["x"], outputs=["y"], **kwargs)
...
for name, axis, kwargs in [("reducemax_dummy_init", [1], {})]:
    axis_init = onnx.helper.make_tensor(
        name="a",
        data_type=onnx.TensorProto.INT64,
        dims=(len(axis),),
...
for name, axis, kwargs in [("reducemax_other_node", [1], {})]:
    ...
    node = onnx.helper.make_node(
        "ReduceMax", inputs=["x", "axis"], outputs=["y"], **kwargs
    )
