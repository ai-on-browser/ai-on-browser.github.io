...
min_node = onnx.helper.make_node("Min", inputs=["x1", "x2"], outputs=["y"])

graph_def = onnx.helper.make_graph(
    nodes=[min_node], name="graph", inputs=[X1, X2], outputs=[Y], initializer=[]
)
model_def = onnx.helper.make_model(graph_def, producer_name="onnx-example")
onnx.checker.check_model(model_def)

onnx.save(model_def, f"{os.path.dirname(__file__)}/min.onnx")
