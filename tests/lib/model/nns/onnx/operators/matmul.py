...
C_init = onnx.helper.make_tensor(
    name="c", data_type=onnx.TensorProto.FLOAT, dims=(10, 3), vals=[1] * 30
)
const_node = onnx.helper.make_node(
    "Constant", inputs=[], outputs=["const"], value=C_init
)
matmul_node = onnx.helper.make_node("MatMul", inputs=["x", "const"], outputs=["y"])
...
onnx.save(model_def, f"{os.path.dirname(__file__)}/matmul.onnx")
