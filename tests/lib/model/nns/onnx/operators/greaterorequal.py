...
greaterorequal_node = onnx.helper.make_node(
    "GreaterOrEqual", inputs=["x1", "x2"], outputs=["y"]
)

graph_def = onnx.helper.make_graph(
    nodes=[greaterorequal_node],
    name="graph",
    inputs=[X1, X2],
    outputs=[Y],
    initializer=[],
...
onnx.save(model_def, f"{os.path.dirname(__file__)}/greaterorequal.onnx")
