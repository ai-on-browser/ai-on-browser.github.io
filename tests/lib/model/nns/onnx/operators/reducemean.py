...
for name, axis, kwargs in [
    ("reducemean", [1], {}),
    ("reducemean_not_keepdims", [1], {"keepdims": 0}),
    ("reducemean_noop_with_empty_axes", [1], {"noop_with_empty_axes": 1}),
]:
    ...
    node = onnx.helper.make_node(
        "ReduceMean", inputs=["x", "a"], outputs=["y"], **kwargs
    )
...
for name, kwargs in [("reducemean_no_axis", {})]:
    node = onnx.helper.make_node("ReduceMean", inputs=["x"], outputs=["y"], **kwargs)
...
for name, axis, kwargs in [("reducemean_dummy_init", [1], {})]:
    axis_init = onnx.helper.make_tensor(
        name="a",
        data_type=onnx.TensorProto.INT64,
        dims=(len(axis),),
...
for name, axis, kwargs in [("reducemean_other_node", [1], {})]:
    ...
    node = onnx.helper.make_node(
        "ReduceMean", inputs=["x", "axis"], outputs=["y"], **kwargs
    )
