...
describe('layer', () => {
	...
	test('toObject', () => {
		...
		expect(obj).toEqual({ type: 'swish', beta: 1 })
	})

	test('fromObject', () => {
		const layer = SwishLayer.fromObject({ type: 'swish' })
		expect(layer).toBeInstanceOf(SwishLayer)
	})
})

describe('nn', () => {
	test('calc', () => {
		const net = NeuralNetwork.fromObject([{ type: 'input' }, { type: 'swish' }])
