...
describe('layer', () => {
	describe('construct', () => {
		...
		test('invalid channel', () => {
			expect(() => new AveragePoolLayer({ kernel: 2, channel_dim: 2 })).toThrow('Invalid channel dimension')
		})
	})
	...
	describe('calc', () => {
		...
		test('kernel:[2, 2] channel dim: 1', () => {
			const layer = new AveragePoolLayer({ kernel: [2, 2], channel_dim: 1 })
				...
				for (let j = 0; j < 2; j++) {
					for (let k = 0; k < 2; k++) {
						for (let c = 0; c < x.sizes[1]; c++) {
							...
							for (let s = 0; s < 2; s++) {
								for (let t = 0; t < 2; t++) {
									sumval += x.at(i, c, j * 2 + s, k * 2 + t)
									count++
								}
							}
							expect(y.at(i, c, j, k)).toBeCloseTo(sumval / count)
						}
					}
				}
			}
		...
		test('invalid padding size', () => {
			const layer = new AveragePoolLayer({ kernel: 2, padding: [1] })
				...
				for (let c = 0; c < x.sizes[1]; c++) {
					...
					for (let j = 0; j < 2; j++) {
						for (let k = 0; k < 2; k++) {
							for (let s = 0; s < 2; s++) {
								for (let t = 0; t < 2; t++) {
									a.operateAt([j * 2 + s, k * 2 + t], v => v + bo.at(i, c, j, k) / 4)
								}
							}
						}
					}
