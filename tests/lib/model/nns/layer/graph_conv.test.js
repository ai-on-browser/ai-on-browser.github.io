...
describe('layer', () => {
	test('construct', () => {
		const layer = new GraphConvolutionalLayer({ out_size: 5 })
		expect(layer).toBeDefined()
	})

	test('dependentLayers', () => {
		const layer = new GraphConvolutionalLayer({ w: 'w', b: 'b', activation: { type: 'clip', min: 'min' } })
	...
	describe('calc', () => {
		...
		test('string activation', () => {
			const layer = new GraphConvolutionalLayer({ out_size: 5, activation: 'sigmoid' })
		...
		test('object activation', () => {
			const layer = new GraphConvolutionalLayer({ out_size: 5, activation: { type: 'sigmoid' } })
	...
	test('toObject', () => {
		...
		expect(obj).toEqual({ type: 'graph_conv', l1_decay: 0, l2_decay: 0, out_size: 5 })
	})

	test('fromObject', () => {
		const orglayer = new GraphConvolutionalLayer({})
		const layer = GraphConvolutionalLayer.fromObject(orglayer.toObject())
		expect(layer).toBeInstanceOf(GraphConvolutionalLayer)
	})
})

describe('nn', () => {
	test('calc', () => {
		const net = NeuralNetwork.fromObject([{ type: 'input' }, { type: 'graph_conv', out_size: 5 }])
		const graphs = [Graph.fromName('butterfly'), Graph.fromName('diamond')]
		for (let i = 0; i < graphs.length; i++) {
			for (let k = 0; k < graphs[i].order; k++) {
				graphs[i].nodes[k] = Matrix.randn(1, 7)
	...
	test('grad', () => {
		const net = NeuralNetwork.fromObject(
			[{ type: 'input' }, { type: 'graph_conv', out_size: 2 }, { type: 'readout' }],
			'mse',
			'adam'
		)
		const graphs = [Graph.fromName('butterfly'), Graph.fromName('diamond')]
		...
		const net = NeuralNetwork.fromObject(
			[
				{ type: 'input' },
				{
					type: 'graph_conv',
					out_size: 2,
					w: Matrix.random(5, 2, -0.1, 0.1).toArray(),
	...
	test('string parameters', () => {
		const net = NeuralNetwork.fromObject(
			[
				{ type: 'input', name: 'in' },
				{ type: 'variable', value: Matrix.randn(7, 2), name: 'w' },
				{ type: 'variable', value: Matrix.randn(1, 2), name: 'b' },
				{ type: 'graph_conv', input: 'in', w: 'w', b: 'b' },
				{ type: 'readout' },
			],
			'mse',
			'adam'
	...
	test('grad decay', { retry: 3, timeout: 10000 }, () => {
		const net = NeuralNetwork.fromObject(
			[{ type: 'input' }, { type: 'graph_conv', out_size: 3, l2_decay: 0.001 }, { type: 'readout' }],
			'mse',
			'adam'
		)
		const graphs = [Graph.fromName('butterfly'), Graph.fromName('diamond')]
