...
import MinLayer from '../../../../../lib/model/nns/layer/min.js'

describe('layer', () => {
	test('construct', () => {
		const layer = new MinLayer({})
	...
	describe('calc', () => {
		test('matrix', () => {
			...
			for (let i = 0; i < x1.rows; i++) {
				for (let j = 0; j < x1.cols; j++) {
					expect(y.at(i, j)).toBeCloseTo(Math.min(x1.at(i, j), x2.at(i, j)))
				}
			}
		})
		...
		test('sub matrix', () => {
			...
			for (let i = 0; i < x1.rows; i++) {
				for (let j = 0; j < x1.cols; j++) {
					expect(y.at(i, j)).toBeCloseTo(Math.min(x1.at(i, j), x2.at(0, j)))
				}
			}
		})
			...
			for (let i = 0; i < x1.sizes[0]; i++) {
				for (let j = 0; j < x1.sizes[1]; j++) {
					for (let k = 0; k < x1.sizes[2]; k++) {
						expect(y.at(i, j, k)).toBeCloseTo(Math.min(x1.at(i, j, k), x2.at(i, j, k)))
					}
				}
			}
		})
		...
		test('sub tensor', () => {
			...
			for (let i = 0; i < x1.sizes[0]; i++) {
				for (let j = 0; j < x1.sizes[1]; j++) {
					for (let k = 0; k < x1.sizes[2]; k++) {
						expect(y.at(i, j, k)).toBeCloseTo(Math.min(x1.at(i, j, k), x2.at(0, 0, k)))
					}
				}
			}
		})
	...
	test('toObject', () => {
		...
		expect(obj).toEqual({ type: 'min' })
	})

	test('fromObject', () => {
		const layer = MinLayer.fromObject({ type: 'min' })
		expect(layer).toBeInstanceOf(MinLayer)
	})
})
	...
	test('calc', () => {
		...
		for (let i = 0; i < a.rows; i++) {
			for (let j = 0; j < a.cols; j++) {
				expect(y.at(i, j)).toBeCloseTo(Math.min(a.at(i, j), b.at(i, j), c.at(i, j)))
			}
		}
	})
