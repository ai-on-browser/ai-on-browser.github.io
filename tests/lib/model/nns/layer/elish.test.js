...
describe('layer', () => {
	test('construct', () => {
		const layer = Layer.fromObject({ type: 'elish' })
			...
			for (let i = 0; i < x.rows; i++) {
				for (let j = 0; j < x.cols; j++) {
					expect(y.at(i, j)).toBeCloseTo(
						x.at(i, j) >= 0
							? x.at(i, j) / (1 + Math.exp(-x.at(i, j)))
							: (Math.exp(x.at(i, j)) - 1) / (1 + Math.exp(-x.at(i, j)))
					)
				}
			}
		})
				...
				for (let j = 0; j < x.sizes[1]; j++) {
					for (let k = 0; k < x.sizes[2]; k++) {
						expect(y.at(i, j, k)).toBeCloseTo(
							x.at(i, j, k) >= 0
								? x.at(i, j, k) / (1 + Math.exp(-x.at(i, j, k)))
								: (Math.exp(x.at(i, j, k)) - 1) / (1 + Math.exp(-x.at(i, j, k)))
						)
					}
				}
			}
				...
				for (let j = 0; j < x.cols; j++) {
					expect(bi.at(i, j)).toBeCloseTo(
						x.at(i, j) >= 0
							...
							: Math.exp(x.at(i, j)) / (1 + Math.exp(-x.at(i, j))) +
									((Math.exp(x.at(i, j)) - 1) * Math.exp(-x.at(i, j))) /
										(1 + Math.exp(-x.at(i, j))) ** 2
					)
				}
			}
		})
					...
					for (let k = 0; k < x.sizes[2]; k++) {
						expect(bi.at(i, j, k)).toBeCloseTo(
							x.at(i, j, k) >= 0
								...
								: Math.exp(x.at(i, j, k)) / (1 + Math.exp(-x.at(i, j, k))) +
										((Math.exp(x.at(i, j, k)) - 1) * Math.exp(-x.at(i, j, k))) /
											(1 + Math.exp(-x.at(i, j, k))) ** 2
						)
					}
				}
			}
...
describe('nn', () => {
	test('calc', () => {
		const net = NeuralNetwork.fromObject([{ type: 'input' }, { type: 'elish' }])
