...
describe('layer', () => {
	...
	describe('calc', () => {
		...
		test('tensor', () => {
			...
			const x = Tensor.random([100, 20, 10], 1, 5)
			const y = layer.calc(x)
			for (let i = 0; i < x.sizes[0]; i++) {
				for (let j = 0; j < x.sizes[1]; j++) {
					for (let k = 0; k < x.sizes[2]; k++) {
						expect(y.at(i, j, k)).toBeCloseTo(Math.acosh(x.at(i, j, k)))
					}
				}
			}
		})
	...
	describe('grad', () => {
		...
		test('tensor', () => {
			...
			for (let i = 0; i < x.sizes[0]; i++) {
				for (let j = 0; j < x.sizes[1]; j++) {
					for (let k = 0; k < x.sizes[2]; k++) {
						expect(bi.at(i, j, k)).toBeCloseTo(1 / (Math.sqrt(x.at(i, j, k) ** 2 - 1) + 1.0e-4))
					}
				}
			}
		})
