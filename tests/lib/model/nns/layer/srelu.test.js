...
import ShiftedReLULayer from '../../../../../lib/model/nns/layer/srelu.js'

describe('layer', () => {
	test('construct', () => {
		const layer = new ShiftedReLULayer({})
	...
	test('toObject', () => {
		...
		expect(obj).toEqual({ type: 'srelu', d: 0 })
	})

	test('fromObject', () => {
		const layer = ShiftedReLULayer.fromObject({ type: 'srelu' })
		expect(layer).toBeInstanceOf(ShiftedReLULayer)
	})
})

describe('nn', () => {
	test('calc', () => {
		const net = NeuralNetwork.fromObject([{ type: 'input' }, { type: 'srelu' }])
