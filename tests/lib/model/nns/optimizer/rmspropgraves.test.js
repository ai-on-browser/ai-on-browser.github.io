...
import { RMSPropGravesOptimizer } from '../../../../../lib/model/nns/optimizer/rmspropgraves.js'
import Tensor from '../../../../../lib/util/tensor.js'

describe('rmspropgraves', () => {
	test('lr', () => {
		const opt = new RMSPropGravesOptimizer(0.1)
	...
	describe('delta', () => {
		test('scalar', () => {
			...
			for (let i = 0; i < 10; i++) {
				...
				r = r * beta + v * (1 - beta)
				s = s * beta + v ** 2 * (1 - beta)
				expect(d).toBeCloseTo((0.1 * v) / Math.sqrt(s - r ** 2))
			}
		})
		...
		test('matrix', () => {
			...
			for (let t = 0; t < 10; t++) {
				...
				s.broadcastOperate(mat, (a, b) => a * beta + b ** 2 * (1 - beta))
				for (let i = 0; i < mat.rows; i++) {
					for (let j = 0; j < mat.cols; j++) {
						expect(d.at(i, j)).toBeCloseTo((0.1 * mat.at(i, j)) / Math.sqrt(s.at(i, j) - r.at(i, j) ** 2))
					}
				}
			}
		})
					...
					for (let j = 0; j < mat.sizes[1]; j++) {
						for (let k = 0; k < mat.sizes[2]; k++) {
							expect(d.at(i, j, k)).toBeCloseTo(
								(0.1 * mat.at(i, j, k)) / Math.sqrt(s.at(i, j, k) - r.at(i, j, k) ** 2)
							)
						}
					}
				}
