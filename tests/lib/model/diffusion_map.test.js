...
describe('dimensionality reduction', () => {
	test.each([undefined, 'gaussian', { name: 'gaussian' }])('kernel %j', { retry: 10, timeout: 30000 }, kernel => {
		const x = Matrix.concat(Matrix.random(40, 5, -2, 1), Matrix.random(40, 5, 3, 5)).toArray()

		const y = new DiffusionMap(3, 2, kernel).predict(x)
	...
	test('custom kernel', () => {
		...
		const y = new DiffusionMap(2, 2, (x, y) => 1 / x.reduce((s, v, i) => s + (v - y[i]) ** 2, 0)).predict(x)
		expect(y[0]).toHaveLength(2)
		const q = coRankingMatrix(x, y, 20, 20)
