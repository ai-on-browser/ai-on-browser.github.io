...
describe('anomaly detection', () => {
	...
	test('initialize', async () => {
		...
		await modelSelectBox.selectOption('nof')
		const methodMenu = await page.waitForSelector('#ml_selector #method_menu')
		const buttons = await methodMenu.waitForSelector('.buttons')

		const auto = await buttons.waitForSelector('input:nth-of-type(1)')
		await expect((await auto.getProperty('value')).jsonValue()).resolves.toBe('on')
		...
		await expect((await threshold.getProperty('value')).jsonValue()).resolves.toBe('1.8')
	})
