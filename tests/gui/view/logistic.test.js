...
describe('classification', () => {
	...
	test('initialize', async () => {
		...
		await expect(methods.inputValue()).resolves.toBe('multinomial')
		const iteration = buttons.locator('select:nth-of-type(2)')
		await expect(iteration.inputValue()).resolves.toBe('1')
		const rate = buttons.locator('input:nth-of-type(2)')
		await expect(rate.inputValue()).resolves.toBe('0.1')
		const l1 = buttons.locator('input:nth-of-type(3)')
		await expect(l1.inputValue()).resolves.toBe('0')
		const l2 = buttons.locator('input:nth-of-type(4)')
		await expect(l2.inputValue()).resolves.toBe('0')
	})
