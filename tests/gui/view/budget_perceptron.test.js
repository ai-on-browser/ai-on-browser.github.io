...
describe('classification', () => {
	...
	beforeEach(async () => {
		...
		await modelSelectBox.selectOption('budget_perceptron')
	})
	...
	test('initialize', async () => {
		...
		await expect(beta.getAttribute('value')).resolves.toBe('1')
		const budgets = await buttons.waitForSelector('input:nth-of-type(2)')
		await expect(budgets.getAttribute('value')).resolves.toBe('10')
