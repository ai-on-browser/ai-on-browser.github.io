...
describe('classification', () => {
	...
	test('initialize', async () => {
		...
		await modelSelectBox.selectOption('projectron')
		const methodMenu = await page.waitForSelector('#ml_selector #method_menu')
		const buttons = await methodMenu.waitForSelector('.buttons')

		const type = await buttons.waitForSelector('select:nth-of-type(1)')
		await expect((await type.getProperty('value')).jsonValue()).resolves.toBe('Projectron')
		...
		await expect((await eta.getProperty('value')).jsonValue()).resolves.toBe('0')
