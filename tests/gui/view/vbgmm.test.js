...
describe('clustering', () => {
	...
	test('initialize', async () => {
		...
		await expect(alpha.getAttribute('value')).resolves.toBe('0.001')
		const beta = await buttons.waitForSelector('input:nth-of-type(2)')
		await expect(beta.getAttribute('value')).resolves.toBe('0.001')
		const k = await buttons.waitForSelector('input:nth-of-type(3)')
		await expect(k.getAttribute('value')).resolves.toBe('10')
