...
describe('classification', () => {
	...
	beforeEach(async () => {
		...
		await modelSelectBox.selectOption('bpa')
	})
	...
	test('initialize', async () => {
		...
		const version = await buttons.waitForSelector('select:nth-of-type(3)')
		await expect((await version.getProperty('value')).jsonValue()).resolves.toBe('simple')
		const c = await buttons.waitForSelector('input:nth-of-type(1)')
		await expect(c.getAttribute('value')).resolves.toBe('1')
		const b = await buttons.waitForSelector('input:nth-of-type(2)')
