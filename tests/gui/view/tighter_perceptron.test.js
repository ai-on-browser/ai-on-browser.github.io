...
describe('classification', () => {
	...
	beforeEach(async () => {
		...
		await modelSelectBox.selectOption('tighter_perceptron')
	})
	...
	test('initialize', async () => {
		...
		await expect(beta.getAttribute('value')).resolves.toBe('0.1')
		const p = await buttons.waitForSelector('input:nth-of-type(2)')
		await expect(p.getAttribute('value')).resolves.toBe('10')
