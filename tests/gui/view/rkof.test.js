...
describe('anomaly detection', () => {
	...
	test('initialize', async () => {
		...
		await modelSelectBox.selectOption('rkof')
		...
		await expect((await h.getProperty('value')).jsonValue()).resolves.toBe('0.2')
		const alpha = await buttons.waitForSelector('input:nth-of-type(3)')
		...
		await expect((await threshold.getProperty('value')).jsonValue()).resolves.toBe('1.3')
	}, 10000)
