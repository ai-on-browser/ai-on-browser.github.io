...
class DataPointStarPlotter {
	...
	cx(value) {
		...
		return value !== undefined
			? this.g.attr('transform', 'translate(' + this._c[0] + ', ' + this._c[1] + ')') && this
			: this._c[0]
	}
...
class DataPoint {
	...
	move(to, duration = 1000) {
		...
		this._plotter.transition().duration(duration).cx(this.vector.value[0]).cy(this.vector.value[1])
...
class DataCircle {
	...
	move(duration = 1000) {
		this.item.transition().duration(duration).attr('cx', this._at.at[0]).attr('cy', this._at.at[1])
	}
