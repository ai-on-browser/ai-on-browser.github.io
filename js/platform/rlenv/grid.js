import GridMazeRLEnvironment from '../../../lib/rl/grid.js'
...
export default class GridMazeRenderer extends GridMazeRLEnvironment {
	...
	init(r) {
		...
		for (let i = 0; i < this._size[0]; i++) {
			...
			for (let j = 0; j < this._size[1]; j++) {
				const g = (this._render_blocks[i][j] = base
					.append('g')
					.classed('grid', true)
					.attr('stroke-width', 1)
					.attr('stroke', 'black')
			...
			for (let i = 0; i < this._size[0]; i++) {
				...
				for (let j = 0; j < this._size[1]; j++) {
					...
					if (this._show_max) {
						...
						this._render_blocks[i][j].select('text.value').text(`${bm}`.slice(0, 6))
					} else {
						this._render_blocks[i][j].selectAll('polygon').each(function (e, k) {
