import ARIMA from '../../lib/model/arima.js'
...
export default function (platform) {
	...
	const fitModel = () => {
		...
		if (!model) {
			model = []
			for (let f = 0; f < tx[0].length; f++) {
				model[f] = new ARIMA(p.value, d.value, q.value)
			}
		}
		const pred = []
	...
	const d = controller.input.number({ label: 'd', min: 0, max: 1000, value: 1 })
