...
`;const e=new h(t);let i=null;const c=t.task,l=a(()=>{if(c==="FS"){i.fit(t.trainInput,t.trainOutput);const s=i.importance().map((n,o)=>[n,o]);s.sort((n,o)=>o[0]-n[0]);const u=t.dimension,p=s.map(n=>n[1]).slice(0,u),$=k.fromArray(t.trainInput);t.trainResult=$.col(p).toArray()}else{i.fit(t.trainInput,t.trainOutput);const s=i.predict(t.testInput(4));t.testResult(s)}},"fitModel"),r=e.select(["CD","ISTA","LARS"]),d=e.select({label:"lambda = ",values:[0,1e-4,.001,.01,.1,1,10,100]});e.stepLoopButtons().init(()=>{i=new b(+d.value,r.value),t.init()}).step(l).epoch()}a(g,"default");
