class ElasticNet2dWorker extends BaseWorker {
	constructor(classes) {
...
var dispElasticNet2d = function(elm) {
	...
	let model = new ElasticNet2dWorker();
	let isRunning = false;
	let epoch = 0;
	...
	elm.select(".buttons")
		...
		.on("click", () => {
			model.initialize(2, 1, +elm.select(".buttons [name=lambda]").property("value"), +elm.select(".buttons [name=alpha]").property("value"));
			tileLayer.selectAll("*").remove();
