class MLPWorker extends BaseWorker {
	...
	initialize(classes, hidden_size, activation) {
		this._postMessage({
			"mode": "init",
			"type": "classifier",
			"features": 2,
...
var dispMLP = function(elm) {
	...
	elm.select(".buttons")
		...
		.on("click", () => {
			...
			model.initialize(model_classes, hidden_number, activation);
			tileLayer.selectAll("*").remove();
		});
	elm.select(".buttons")
		.append("span")
